...
py::array_t<double> gaotest(py::array_t<long long>& input1, py::array_t<long long>& input2,py::array_t<long long>& input3, py::array_t<long long>& input4) {
    ...
    int nn1=buf1.shape[0];//posuer个数
    int nn2=buf2.shape[0];//positem个数
    int n=buf3.shape[0];//user set数目
    int n1=buf4.shape[0];//user set数目
    int ch=buf3.shape[1];//topK最大值
    int m=buf4.shape[1];//topK
 ...
 //   cout<<nn1<<' '<<nn2<<' '<<dd1<<' '<<dd2<<endl;
   ...
   // cout<<nn1<<' '<<nn2<<' '<<n<<' '<<ch<<' '<<m<<endl;  
    ...
    for(int i=0;i<n;i++)
    num[i]=0;//每个user的hit数目
    
    logsum[0]=0;
    for(int j=1;j<=m;j++)
    {
       logsum[j]=logsum[j-1]+1.0/log2(j+1);//算了全是pos情况下的DCG分母
    }
    ...
    for(int i=0;i<nn1;i++)
    {
        ...
        num[x]++;//只要posuser里出现了这个数字，他就+1
        long long y=testi(i);
        long long code=(x<<30)+y;
        mt[code]=1;//MT[code]里存了所有的正样本ui对，2的30的uID+iID的数组idex，值是1
    }
   for(int i=0;i<n;i++)
   {
       idcg[i]=logsum[num[i]];
 ...
 // cout<<nn1<<' '<<nn2<<' '<<n<<' '<<ch<<' '<<m<<endl;  
    ...
    for(long long i=0;i<n;i++)
    {
       // cout<<i<<endl;
        ...
        cao++;//不是没有pos的user，就给要当分母的user总数+1
        double hit=0;
        double ndie=0;
        for(int j=0;j<ch;j++)
        {
 ...
 //   cout<<anpre<<' '<<anre<<' '<<anndcg<<' '<<anmrr<<' '<<cao<<endl;
  ...
  //  cout<<anpre<<' '<<anre<<' '<<anndcg<<' '<<anmrr<<' '<<cao<<endl;
   ...
   auto result = py::array_t<double>(6);
