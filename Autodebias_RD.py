...
def para(args): 
    if args.dataset == 'yahooR3': 
        ...
        args.auto_temperature = 10
        args.Gama_Auto = 3
        args.w1_ad_lr = 0.1
    elif args.dataset == 'coat':
        args.training_args = {'batch_size': 128, 'epochs': 500, 'patience': 60, 'block_batch': [64, 64]}
        args.base_model_args = {'emb_dim': 10, 'learning_rate': 0.0005, 'imputaion_lambda': 0.05, 'weight_decay': 1}
        args.weight1_model_args = {'learning_rate': 0.0001, 'weight_decay': 1e-4}
...
def both_test(loader, model_name, testname, K = 5, dataset = "None", device='cuda'):
    ...
    U = test_results['UAUC']
    N = test_results['NDCG']
    print(f'The performance of RD-Autodebias on uniform data is: UAUC = {U} NDCG@5 = {N}')
    return test_results
...
def train_and_eval(train_data, unif_train_data, val_data, test_data, device = 'cuda',
    ...
    # Load the pre-learned propensity of AutoDebias model 
    ...
    for epo in range(training_args['epochs']):
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader): 
                ...
                if epo > 20:
                    ...
                    weight1_ad = torch.exp(torch.nn.ReLU()(weight1_ad/args.auto_temperature))
                    
                    weight2_model_ad.train()
                    weight2_ad = weight2_model_ad(users_all, items_all,(train_dense[users_all,items_all]!=0)*1)
                    weight2_ad = torch.exp(torch.nn.ReLU()(weight2_ad/args.auto_temperature))
                    ...
                    # adversal learning on biased data
                    y_hat_ad = one_step_model_ad(users_train, items_train)
                    loss_ad = -1 * sum_criterion(y_hat_ad, y_train)
                    ...
                    weight1_after_adv = torch.exp(torch.nn.ReLU()(weight1_after_adv/args.auto_temperature))

                    weight2_after_adv = weight2_model_ad(users_all, items_all,(train_dense[users_all,items_all]!=0)*1)
                    weight2_after_adv = torch.exp(torch.nn.ReLU()(weight2_after_adv/args.auto_temperature))

                    # inner bound
                    ancor_1 = stable_weight1_model(users_train, items_train,(y_train==1)*1)
                    ancor_1 = torch.exp(torch.nn.ReLU()(ancor_1/args.auto_temperature))
                    ancor_2 = stable_weight2_model(users_all, items_all,(train_dense[users_all,items_all]!=0)*1)
                    ancor_2 = torch.exp(torch.nn.ReLU()(ancor_2/args.auto_temperature))
        ...
        print('Epoch: {0:2d} / {1}, Traning: {2}, Validation: {3}'.
                format(epo, training_args['epochs'], ' '.join([key+':'+'%.5f'%train_results[key] for key in train_results]), 
                    ' '.join([key+':'+'%.5f'%val_results[key] for key in val_results])))
